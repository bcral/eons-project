...
contract EonsVault is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of EONSs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accEonsPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws  tokens to a pool. Here's what happens:
        //   1. The pool's `accEonsPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. EONSs to distribute per block.
        uint256 accEonsPerShare; // Accumulated EONSs per share, times 1e12. See below.
    ...
    // Update the given pool's EONSs allocation point. Can only be called by the owner.
    // Note contract owner is meant to be a governance contract allowing EONS governance consensus
    ...
    // View function to see pending EONSs on frontend.
    function pendingEons(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
    ...
    // ----
    // Function that adds pending rewards, called by the EONS token.
    // ----
    uint256 private eonsBalance;

    function addPendingRewards(uint256 _) public {
        uint256 newRewards = eons.balanceOf(address(this)).sub(eonsBalance);

        if (newRewards > 0) {
            eonsBalance = eons.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid)
        internal
        returns (uint256 eonsRewardWhole)
    {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        eonsRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 eonsRewardFee = eonsRewardWhole.mul(DEV_FEE).div(10000);
        uint256 eonsRewardToDistribute = eonsRewardWhole.sub(eonsRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(eonsRewardFee);

        pool.accEonsPerShare = pool.accEonsPerShare.add(
            eonsRewardToDistribute.mul(1e12).div(tokenSupply)
        );
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accEonsPerShare).div(1e12);
    ...
    function depositFor(
        ...
        user.rewardDebt = user.amount.mul(pool.accEonsPerShare).div(1e12); /// This is deposited for address
    ...
    function _withdraw(
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid, from); // Update balances of from this is not withdrawal but claiming EONS farmed
    ...
    function updateAndPayOutPending(uint256 _pid, address from) internal {
        ...
        uint256 pending = user.amount.mul(pool.accEonsPerShare).div(1e12).sub(
            user.rewardDebt
        );

        if (pending > 0) {
            safeEonsTransfer(from, pending);
        }
    }
    ...
    function safeEonsTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > eonsBal) {
            ...
            eonsBalance = eons.balanceOf(address(this));
        } else {
    ...
    function transferDevFee() public {
        ...
        uint256 eonsBal = eons.balanceOf(address(this));
        if (pending_DEV_rewards > eonsBal) {
            eons.transfer(devaddr, eonsBal);
            eonsBalance = eons.balanceOf(address(this));
        } else {
            eons.transfer(devaddr, pending_DEV_rewards);
            eonsBalance = eons.balanceOf(address(this));
        }
